...
The API has 7 endpoints. The blue button on each endpoint tells us that *all* endpoints are `GET` endpoints. We don't have any of the other request types we discussed in @sec-type-requests. This is important because depending on the type of request you might need to do additional things (for `POST` you'll need to add a request body, for example). In this book we'll only work with `GET` requests because this is the type of request you'll typically encounter when gathering data. However, be aware that you might encounter other types of requests out there in the wild.
...
## Understanding Amazon Books {#sec-understand-amazon-books}
...
req_res
```
The `GET` request was updated with the endpoint URL and we're ready to perform the request. Let's make the request and check out the response:
...
resp
```
There we go, the name of the product is `r unique(resp$name)[1]`. Quite a hassle, eh? In the exercises section you'll be asked to come up with a way to automate this entire process such that it's very easy to check which products a user bought.
...
2. From all products that were bought (this means all users who bought any products), which are the most common tags? Similarly to the previous exercise, you need to end up with a vector with the frequency of categories throughout all products like this one:
