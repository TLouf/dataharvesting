...
- One-off scraping will solve many problems and is usually the first starting point

- However, you might need this to collect data that is changing constantly

- Sometimes you want to gather info that disappears
...
With the directory created, we copy the R script and check that it is there with `ls`
...
The output means you have no scheduled scripts on your computer.
...
1.  A schedule
2.  What to execute, here: `Rscript ~/newspaper/newspaper_scraper.R`
...
- Will get messy because it'll append the same results in the CSV file continuously.
...
- Setting up production-ready scrapers is difficult: databases, interactivity, persistence, avoid bans, saving data real time, etc..
