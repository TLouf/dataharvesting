...
APIs are designed to work programmatically, not through a user interface.
...
- Construct logic in your program: results from an endpoint are used as input to another endpoint and checks are provided along the way

- Build applications that request data in real time

## A Primer on APIs

This chapter is hands-on. We focus on programmatically accessing an API.
...
- Note that when learning about a new API online you **won't** have to launch any API.
...
- How do we make a request with this info?

## A Primer on APIs

- A base URL. In this case, it's <http://localhost:2234>. **Yours will be different because you launched it on your local computer**.
...
Construct URL query programmatically:
...
See how we can "add" as many things to our "request"? These things might come in the headers or defined in R.
...
- How are project ideas coming along? Should confirm with me by email. The deadline to submit project ideas is next week. I advise starting to work on the project **now**. [Here](https://docs.google.com/spreadsheets/d/11QmdgRXvbwtN9hSOwVyw9-iVjrN-yeRq_BOudidAnX8/edit#gid=73208201).
