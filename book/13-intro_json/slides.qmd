...
- Important: allows you to build complex hierarchical structures
...
- Each `key:value` can contain numbers or strings (for now)
...
JSONs are `key:value` pairs and the value itself can be an array with other key/value pairs.
...
- The most important tool for working with JSONs is subsetting
...
2.  List column has two data frames and an empty slot
...
- If all objects are of different classes (`data.frame`, `vectors`, etc..), then `unnest` will fail.

- If all objects within the list are of the same class, it will combine all of them into a proper column or 'unpack' its values.
...
The main problem you'll encounter with JSONs is that you're trying to parse some JSON that has many **nested arrays** and some of these arrays are not **compatible for unnesting** so you'll have to submerge yourself into these nested arrays and fix whatever data you want to extract.
...
Before understanding a JSON in depth, it's not a bad idea to try to read it:
...
- General pattern: nested structures
...
- Work on the project should begin now. Submissions can be done from now on. Final deadline for the project is in two weeks.
