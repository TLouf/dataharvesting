...
Use `|` to separate regex. Here we use it to say: replace either `maps, ` or ` century`:
...
::::columns
:::{.column width="80%"}
Anchors are `^` for beginning and `$` for ending.
How to remember? If you start with power, you end up with money (_a reverse Tony Montana philosophy, if you will_).
:::

:::{.column width="10%"}
![](images/tony_montana_ud.png){width="200px"}
:::
::::
...
`ul` tags are very common and subsetting only for `ul` tags will bring many matches. XPath is the topic of the next chapter but bear with me.
...
We have 16 kings/queens but there are two slots that separate them by houses: House of Valois and House of Bourbon.
