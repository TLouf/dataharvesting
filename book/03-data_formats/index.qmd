...
knitr::include_graphics(file.path(main_dir, "xml_one.png"))
```
...
```{r, out.width = "30%", echo = FALSE}
knitr::include_graphics(file.path(main_dir, "xml_two.png"))
```

Similarly, you'll find that the `<carol>` tag is also matched by a `</carol>` finishing tag.

```{r, out.width = "30%", echo = FALSE}
knitr::include_graphics(file.path(main_dir, "xml_three.png"))
```
...
```{r, echo = FALSE}
knitr::include_graphics(file.path(main_dir, "html_ex1.png"))
```
...
knitr::include_graphics(file.path(main_dir, "html_ex2.jpg"))
```
...
Wait, why didn't this work? Well, if you look at the output of `child_xml`, we have two nodes which are for `<jason>` and `<carol>`.
...
However, in most cases the 'addresses' used in `xml_find_all` come from a separate language called XPath (in fact, the 'address' we've been looking at **is** XPath). XPath is a complex language (such as regular expressions for strings) which we'll cover in @sec-xpath-chapter.
...
5. The output of all the previous exercises has been either an `xml_nodeset` or an `html_document` (you can read it at the top of the print out of your results):
