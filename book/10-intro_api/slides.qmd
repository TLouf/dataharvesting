...
> accepting the agreements of using this \'portal\', can access much of the data they can see on a company's website (but not necessarily what you\'re looking for, it is their prerogative).
...
- APIs have endpoints which return specific data
...
3.  This endpoint has two codes: `200` which means it is OK and `500` which means that something wrong happened.
...
- The docs of the REST API document parameters of each endpoint.

- Many REST APIs need authentication tokens for making requests.

- REST API responses return status codes and headers
