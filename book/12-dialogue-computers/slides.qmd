...
- Doesn't end there: *every website in the world has a computer behind it that hosts the data and information shown on the website*.
...
- You've made thousands of requests in your life without even knowing it.
...
What do the instructions of a request look like?
...
As is common for human interactions, a dialogue is between two parties. The computer/server responds back like this:
...
Most common request `GET`. Your browser makes thousands of these every week. However, there are many more:

- `GET`: retrieves whatever information is identified by the URL

- `POST`: requests data just as `GET` but allows you to enclose data in the 'body' of the request.
...
2.  Your browser sends a `GET` request to the servers behind the website, receives a response, and renders the result.

3.  You spend about 15 minutes choosing which plate you're gonna order.

4.  Click on 'submit order'.

5.  You just sent a `POST` request.

The `POST` request contains all the details of your order which are saved on the servers of the website and allow people in the kitchen to start preparing your food.
...
- **Read the terms of service**: companies are sharing data freely over the web but this doesn't mean you can do anything with it.
