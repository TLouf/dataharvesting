...
- XPath also allows you to pick by order of appearance
...
In reality, we won't know the exact address of what we're looking for but rather some vague intuitions about the node. For that we use `*`:
...
The result is not the `dansimmons` tag but all its children, regardless of whether they are `<book>` tags or any other tag
...
1. Find all `<book>` tags
2. Which have an attribute `price` set to `yes`
3. And are *descendants* (but not necessarily direct child, because of the `//`) of the `<dansimmons>` tag.
...
![](images/elpais_science_main.png){fig-align="center"}
...
**Overall: Find all `<a>` tags for which the `@href` attribute contains the text 'science' which are descendants of the `<section>` tag with two tags in between.**
...
Five sections, mostly entertainment related except for the first one which is the front page ('apertura' is something like 'opening')
...
- [The course's cookbook](./index.html#sec-xpath-cookbook)
...
- Read and complete the exercises of chapter 6
